...
public class ListaDupla<T> {
	...
	public No<T> buscarNo(T valor) {
		No<T> buffer = inicio;

		while (buffer != null) {
			if (buffer.getValor().equals(valor)) {
				return buffer;
			}
			buffer = buffer.getProximo();
		}
	...
	public void adicionarPessoa(String nome) {
		...
		No<T> no = buscarNo((T) letra);
		if (no == null) {
			append((T) letra);
			no = fim;
		}
		no.getLista().append(nome);
	}
	...
	public void removerPessoa(String nome) {
		...
		if (no == null) return;
		try {
			int index = no.getLista().index(nome);
			no.getLista().remove(index);
			if (no.getLista().total() == 0) {
				int indexNo = index((T) letra);
				remove(indexNo);
			}
		} catch (Exception e) {
	...
	public String toString() {
		...
		while (buffer != null) {
			if (buffer != this.inicio) {
				builder.append(",");
			}
			builder.append(buffer.getValor()).append(":").append(buffer.getLista().toString());
			buffer = buffer.getProximo();
		}
